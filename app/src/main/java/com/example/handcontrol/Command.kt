...
enum class Command(val russianCommand: String, val englishCommand: String) {
    ...
    companion object {
        fun getRussianCommandIgnoreCase(command: String): Command? {
            ...
            return values().find { it.russianCommand.toLowerCase(locale) == command.toLowerCase(locale) }
        }

        fun getEnglishCommandIgnoreCase(command: String): Command? {
            val locale = Locale.US
            return values().find { it.englishCommand.toLowerCase(locale) == command.toLowerCase(locale) }
        }
    }
}